...
func (r KustoClusterCustomerManagedKeyResource) clusterIsNotUsingCustomerManagedKey(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) error {
	id, err := parse.ClusterID(state.ID)
	if err != nil {
		return err
	}
